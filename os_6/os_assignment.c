...
static int os_mkdir(const char *path, mode_t mode) {
    DfsPath* full_path = dfs_parse_path(path);

    char *dirName;
    DfsStatus separation_status = dfs_path_pop(full_path, &dirName);
    if (separation_status != DFS_OK) {
        return -ENOENT;
    }


    DfsDir *dir;
    DfsStatus dir_status = dfs_find_dir(root_dir, full_path, &dir);
    if (dir_status != DFS_OK) {
        return -ENOENT; // Error: no entity
    }


    DfsDir *newDir = dfs_create_dir();
    DfsStatus add_status = dfs_add_dir(dir, dirName, newDir);
    if(add_status == DFS_E_ENTRY_ALREADY_EXISTS) {
        return -EEXIST;
    }
    if (add_status != DFS_OK) {
        return -ENOENT; // Error: no entity
    }
    return 0;
}
...
static int os_rmdir(const char *path) {
    DfsDir *dirToDel;
    DfsStatus dir_status = dfs_find_dir_str(root_dir, path, &dirToDel);
    if (dir_status != DFS_OK) {
        return -ENOENT; // Error: no entity
    }
    size_t size = dfs_get_dir_size(dirToDel);
    if (size != 0) {
        return -ENOTEMPTY;
    }
    ...
    DfsDir *parentDir;
    DfsStatus parent_status = dfs_find_dir(root_dir, full_path, &parentDir);
    if (parent_status != DFS_OK) {
        return -ENOENT; // Error: no entity
    }
    DfsStatus remove_status = dfs_remove_dir(parentDir, dirName, NULL);
    if (remove_status != DFS_OK) {
        return -ENOENT; // Error: no entity
    }
...
static int os_write(
    ...
    if(file->length < off + size) {
        file->contents = realloc(file->contents, off + size);
        file->length = off + size;
    }
...
static int os_create(const char *path, mode_t mode, struct fuse_file_info *fi) {
    ...
    char *fileName;
    DfsStatus separation_status = dfs_path_pop(full_path, &fileName);
    if (separation_status != DFS_OK) {
        return -ENOENT;
    }
    ...
    DfsFile *newFile = dfs_create_file();

    DfsStatus add_status = dfs_add_file(parentDir, fileName, newFile);
    if(add_status == DFS_E_ENTRY_ALREADY_EXISTS) {
        return -EEXIST;
    }
    if (add_status != DFS_OK) {
...
static int os_unlink(const char *path) {
    ...
    DfsStatus remove_status = dfs_remove_file(parentDir, fileName, NULL);
    if (remove_status != DFS_OK) {
        return -ENOENT; // Error: no entity
    }
    return 0;
